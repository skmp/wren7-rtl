@ k_blkshot2.s -- as k_blkshot.s, but the probe is in the main code: the handler (length set by P1, no probe)
@ returns to sled2, which probes after 2 adds.
    .include "crt.inc"
_start:
    ...
    mov r11, #0x400
    ldr r12, =0x802D04           @ M
    ldr r13, =sled2              @ where the handler returns (low byte 0: also the zero shift amount)
...
sled2:
    add r7, r7, #1
    add r7, r7, #1
    orr r3, r3, r3, lsl r6       @ r6 = 0x400: shift amount 0; fetch at phase 0, I cycle -> phase 4
    orr r3, r3, r3, lsl r6       @ THE PROBE
sled:
    .rept 2048
    add r7, r7, #1
    .endr
