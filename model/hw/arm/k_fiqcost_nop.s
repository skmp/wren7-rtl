@ k_fiqcost_nop.s -- cost of one FIQ-handler operation on the console.  The sample-interval FIQ (level 1)
@ interrupts an add sled; the handler acknowledges (L read, SCIRE, M), then does 4 - P1 copies of
@ "mov r0, r0", and restarts the sled.  Adds per sample fall by copies x cost / 8.  ITER = samples, r7 = adds.
    .include "crt.inc"
_start:
    ldr r1, =fiq
    sub r1, r1, #0x24
