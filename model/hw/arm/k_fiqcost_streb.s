@ k_fiqcost_streb.s -- cost of one FIQ-handler operation on the console.  The sample-interval FIQ (level 1)
@ interrupts an add sled; the handler acknowledges (L read, SCIRE, M), then does 4 - P1 copies of
@ "str r11, [r8, #-8]", and restarts the sled.  Adds per sample fall by copies x cost / 8.  ITER = samples, r7 = adds.
    .include "crt.inc"
_start:
    ldr r1, =fiq
    sub r1, r1, #0x24
    ...
    ldr r9, P1
    mov r9, r9, lsl #0   @ words to skip = P1 x 1
    mov r10, #1
    mov r11, #0x400
    ldr r12, =0x802D04           @ M
    ldr r13, =sled
